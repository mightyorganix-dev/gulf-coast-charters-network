...
function render() {
  ...
  const list = store.filterTrips({ category, marina, maxPrice, party, q });
  const grid = document.getElementById('trips-grid');
  document.getElementById('trips-count').textContent = `${list.length} curated trip${list.length === 1 ? '' : 's'}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ['f-category', 'f-marina', 'f-price', 'f-party', 'f-q'].forEach((id) => {
